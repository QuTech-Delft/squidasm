...
class LhrProcess:
    def __init__(self, host: Host, program: lp.LhrProgram) -> None:
        self._host = host
    ...
    def execute_program(self) -> Generator[EventExpression, None, Dict[str, Any]]:
        ...
        for instr in program.instructions:
            self._logger.info(f"Interpreting LIR instruction {instr}")
            if isinstance(instr, lp.SendCMsgOp):
                value = memory[instr.arguments[0]]
                self._logger.info(f"sending msg {value}")
                csck.send(value)
            elif isinstance(instr, lp.ReceiveCMsgOp):
                msg = yield from csck.recv()
                msg = int(msg)
                memory[instr.results[0]] = msg
                self._logger.info(f"received msg {msg}")
            elif isinstance(instr, lp.AddCValueOp):
                arg0 = memory[instr.arguments[0]]
                arg1 = memory[instr.arguments[1]]
                memory[instr.results[0]] = arg0 + arg1
            elif isinstance(instr, lp.MultiplyConstantCValueOp):
                arg0 = memory[instr.arguments[0]]
                arg1 = instr.arguments[1]
                memory[instr.results[0]] = arg0 * arg1
            elif isinstance(instr, lp.BitConditionalMultiplyConstantCValueOp):
                arg0 = memory[instr.arguments[0]]
                arg1 = instr.arguments[1]
                cond = memory[instr.arguments[2]]
                if cond == 1:
                    memory[instr.results[0]] = arg0 * arg1
            elif isinstance(instr, lp.AssignCValueOp):
            ...
            elif isinstance(instr, lp.RunSubroutineOp):
                arg_vec: lp.LirVector = instr.arguments[0]
                args = arg_vec.values
                lhr_subrt: lp.LhrSubroutine = instr.attributes[0]
                ...
                for key, mem_loc in lhr_subrt.return_map.items():
                    try:
                        ...
                        pass
            elif isinstance(instr, lp.ReturnResultOp):
...
class Host(ComponentProtocol):
    ...
    def run_lhr_program(
        self, program: lp.LhrProgram, num_times: int
    ) -> Generator[EventExpression, None, None]:
    ...
    def run(self) -> Generator[EventExpression, None, None]:
        ...
        while self._num_pending > 0:
            ...
            if isinstance(self._program, lp.LhrProgram):
                yield from self.run_lhr_program(self._program, 1)
            elif isinstance(self._program, lp.SdkProgram):
