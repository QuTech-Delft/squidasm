...
from util import create_two_node_network, send_int, recv_int, QKDRoutine
...
class SenderProgram(Program):
    ...
    def run(
        ...
        raw_key, error_rate = yield from QKDRoutine.run(
            context, peer_name=self.PEER, is_init=True, num_epr=num_epr, num_test_bits=num_test)
...
class RecieverProgram(Program):
    ...
    def run(
        ...
        raw_key, error_rate = yield from QKDRoutine.run(
            context, peer_name=self.PEER, is_init=False, num_epr=num_epr, num_test_bits=num_test)
