...
def calculate_reference(
    control: DistributedCNOTParams,
    target: DistributedCNOTParams,
    control_ref_state: np.ndarray,
    target_ref_state: np.ndarray,
):
    control_state = get_reference_state(control.phi, control.theta)
    target_state = get_reference_state(target.phi, target.theta)
    control_q, target_q = netsquid.qubits.create_qubits(2)
    netsquid.qubits.qubitapi.assign_qstate(control_q, control_state)
    netsquid.qubits.qubitapi.assign_qstate(target_q, target_state)

    netsquid.qubits.operate([control_q, target_q], netsquid.qubits.operators.CX)

    fid_control = netsquid.qubits.qubitapi.fidelity(control_q, control_ref_state)
    fid_target = netsquid.qubits.qubitapi.fidelity(target_q, target_ref_state)
    return fid_control, fid_target
...
if __name__ == "__main__":
    ...
    netsquid.set_qstate_formalism(netsquid.qubits.qformalism.QFormalism.DM)
